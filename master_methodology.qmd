...
```{r}
# ==============================================================================
# FINAL WORKING ZINB SCRIPT (Corrected Mapping & No Formatting Crashes)
# ==============================================================================
...
run_zinb_incremental <- function(finance_var, latex_name, title_name, label_id, finance_label) {
  ...
  # 1. Define internal names exactly as they appear in the data frame
  fd_c <- paste0(finance_var, "_c")
  ...
  for (i in 1:4) {
    intensity_controls <- paste(controls[1:i], collapse = " + ")
    # The '|' separates the count equation from the zero-inflation (selection) equation
    formula_str <- paste0(
  ...
  # 3. CONSTRUCT COEF_MAP TO RECOVER MISSING COEFFICIENTS
  # modelsummary prepends "count_" and "zero_" to ZINB variables.
  # If you don't list them with these prefixes, they will be hidden.
  cm <- c(
  ...
  # Add dynamic finance names safely with "count_" prefix
  cm[paste0("count_", fd_c)] <- finance_label
  cm[paste0("count_", fd_x)] <- paste0(finance_label, " $\\times$ IPR")
  
  # 4. Generate Output (Removes \num wrappers to prevent PDF crashes)
  modelsummary(
    models,
    output = latex_name,
    stars = TRUE,
    coef_map = cm,
    gof_omit = "IC|Log|Adj|Pseudo|RMSE|R2|AIC|BIC",
    escape = FALSE,
    fmt = 3,
    # This removes the \num{} command that was visible in your .tex files 
    estimate = "{estimate}{stars}" 
  )
  
  # 5. POST-PROCESSING: Keep only tabular guts (Removes \begin{table}, etc.)
  lines <- readLines(latex_name)
  start <- grep("\\\\begin\\{tabular\\}", lines)
  end <- grep("\\\\end\\{tabular\\}", lines)
  if(length(start) > 0 && length(end) > 0) {
    writeLines(lines[start:end], latex_name)
  }
}

# --- Execute for all models ---
run_zinb_incremental("ln_market", "tables/zinb_market.tex", "ZINB: Market Finance", "tbl-zinb-market", "Market Cap")
run_zinb_incremental("ln_bank", "tables/zinb_bank.tex", "ZINB: Bank Development", "tbl-zinb-bank", "Bank Credit")
run_zinb_incremental("ln_vc", "tables/zinb_vc.tex", "ZINB: Venture Capital", "tbl-zinb-vc", "Venture Capital")
```
