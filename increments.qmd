...
```
# core relationships
```{r fig.width=12, fig.height=5, echo= TRUE, eval= TRUE}
# ============================================================
...
# 1. DEFINE YOUR CORE VARIABLES
# Adjust names if yours are slightly different
dv       <- "breakthrough_count"  # The Outcome (Event)
offset   <- "patent_count"        # The Exposure (Opportunity)
main_ivs <- c("ln_market", "ipr_c") # The Predictors

# 2. SUBSET TO COMPLETE CASES (The "Real" Sample)
# Models drop rows with ANY missing value. We must check the size of what remains.
df_ready <- df %>% 
  filter(!is.na(.data[[dv]]), 
         !is.na(.data[[offset]]), 
         if_all(all_of(main_ivs), ~!is.na(.)))

cat("================================================\n")
cat("DATA READINESS REPORT\n")
cat("================================================\n")

# CHECK 1: SAMPLE SIZE
cat("Total Rows in Panel:    ", nrow(df), "\n")
cat("Rows ready for Model:   ", nrow(df_ready), "\n")
cat("Dropout Rate:           ", round((1 - nrow(df_ready)/nrow(df))*100, 1), "%\n")
if(nrow(df_ready) < 500) warning("⚠️ WARNING: Sample size is very low for country-level panels.")

# CHECK 2: THE OUTCOME (Sparsity)
zeros <- sum(df_ready[[dv]] == 0)
prop_zeros <- zeros / nrow(df_ready)
cat("\n--- OUTCOME VARIABLE ---\n")
cat("Total Breakthroughs:    ", sum(df_ready[[dv]]), "\n")
cat("Rows with ZERO events:  ", zeros, "(", round(prop_zeros*100, 1), "% )\n")
if(prop_zeros > 0.90) warning("⚠️ CRITICAL: Data is >90% Zeros. Poisson/Logit will struggle significantly.")

# CHECK 3: THE OFFSET (Opportunity)
# The offset must be correlated with the outcome. If they are totally unrelated, 
# 'breakthroughs' aren't just a subset of 'patents', suggesting a data mismatch.
cor_off <- cor(df_ready[[dv]], df_ready[[offset]], method = "spearman")
cat("\n--- OFFSET VALIDITY ---\n")
cat("Correlation (Outcome vs Offset): ", round(cor_off, 3), "\n")
if(cor_off < 0.3) warning("⚠️ CRITICAL: Breakthroughs and Total Patents are weakly correlated. Are they from the same data source?")

# CHECK 4: VARIANCE (Overdispersion)
mean_y <- mean(df_ready[[dv]])
var_y  <- var(df_ready[[dv]])
cat("\n--- DISTRIBUTION ---\n")
cat("Mean:", round(mean_y, 2), " | Variance:", round(var_y, 2), "\n")
cat("Dispersion Ratio (Var/Mean):", round(var_y/mean_y, 2), "\n")
if((var_y/mean_y) > 5) cat("👉 Suggestion: Use Negative Binomial (MASS::glm.nb or fixest::fenegbin)\n")

# 1. Identify the outlier
outlier_row <- df_est %>% arrange(desc(breakthrough_count)) %>% slice(1)
print(outlier_row)

# 2. Create a "Sanitized" dataset (No NAs, No Outlier)
# We remove the top 1% of extreme values just to be safe, or just the single max.
threshold <- quantile(df_est$breakthrough_count, 0.999) # Top 0.1% cut

df_clean <- df_est %>%
  filter(breakthrough_count < threshold) %>% # Drop the monster outlier
  filter(!is.na(ln_market), !is.na(ipr_c))   # Ensure data exists (Fixes "0 obs" error)

cat("Old Max:", max(df_est$breakthrough_count), "\n")
cat("New Max:", max(df_clean$breakthrough_count), "\n")
```

```{r eval= FALSE, echo= TRUE}
# ============================================================
# BREAKTHROUGH INNOVATION MODELS
# ISO CODE FIX + MERGE + ESTIMATION
# ============================================================
...
# OECD PQI country-year data (ISO-2 codes)
oecd <- fread("data/OECD_country_year_final.csv")

# Monarch panel (true ISO-3 codes + finance/IPR)
monarch <- fread("data/monarch_panel_transformed.csv")

# ------------------------------------------------------------
# FIX COUNTRY CODES (ISO-2 -> ISO-3)
# ------------------------------------------------------------

# IMPORTANT: OECD iso3c is actually ISO-2
oecd <- oecd %>%
  mutate(
    iso2c = iso3c,
    iso3c = countrycode(
...
oecd <- oecd %>% filter(!is.na(iso3c))

# ------------------------------------------------------------
# BASIC VALIDATION
# ------------------------------------------------------------

cat("OECD rows after ISO conversion:", nrow(oecd), "\n")
cat("Unique OECD countries:", n_distinct(oecd$iso3c), "\n")

# ------------------------------------------------------------
# MERGE WITH MONARCH PANEL
# ------------------------------------------------------------
...
cat("Usable observations:", nrow(df_model), "\n")
cat("Countries:", n_distinct(df_model$iso3c), "\n")
cat("Years:", n_distinct(df_model$year), "\n")

# Fail loudly if something is still wrong
stopifnot(nrow(df_model) > 300)

# ------------------------------------------------------------
# CREATE BINARY BREAKTHROUGH EVENT
# ------------------------------------------------------------

df_model <- df_model %>%
  mutate(
    breakthrough_any = as.integer(breakthrough_count > 0)
  )

table(df_model$breakthrough_any)
df_model <- df_model %>% 
  mutate(
    log_patent_count = log(patent_count)
  )
# ============================================================
# MODEL 1: POISSON (INTENSITY, WITH EXPOSURE)
# ============================================================
nrow(df_model)
pois_market <- feglm(
  breakthrough_count ~ ln_market * ipr_c +
    ln_gdp_pc + ln_trade + ln_rd + ln_tertiary |
    iso3c + year,
  offset = ~log_patent_count,
  family = poisson(),
  data = df_model,
  cluster = "iso3c"
)
...
plot_predictions(
  ...
  condition = list("ln_market", "ipr_c"),
  newdata = df_model,
  vcov = FALSE# X-axis = Market, Lines = IPR levels
) +
  labs(
    title = "The Choking Effect of IPR",
    subtitle = "Financial Deepening increases breakthroughs ONLY when IPR is flexible",
    y = "Predicted Breakthrough Count",
    x = "Financial Market Depth (Log)"
  ) +
  theme_minimal()

# ============================================================
# MODEL 2: LOGIT (PROBABILITY OF ANY BREAKTHROUGH)
# ============================================================
...
# ============================================================
# OPTIONAL: BANK & VC VARIANTS
# ============================================================

logit_bank <- feglm(
  breakthrough_any ~ ln_bank * ipr_c +
    ln_gdp_pc + ln_trade + ln_rd + ln_tertiary |
    year,
  family = binomial("logit"),
  data = df_model,
...
```{r eval= FALSE, echo= TRUE}
# ============================================================
# BREAKTHROUGH INNOVATION RATE MODEL
# Negative Binomial / Poisson-ready framework
# ============================================================
...
# OECD PQI country-year data (counts)
pqi <- fread("data/OECD_country_year_final.csv")

# Monarch panel (finance, IPR, controls)
monarch <- fread("data/monarch_panel_transformed.csv")
...
# ------------------------------------------------------------
# REQUIRED:
# - patent_count > 0 (offset must be finite)
# - breakthrough_count observed
# - no missing regressors
...
# Sanity check
cat("Observations used:", nrow(df_est), "\n")

# ------------------------------------------------------------
# POISSON FIXED EFFECTS MODEL (MARKET FINANCE)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# OPTIONAL: BANK & VC CHANNELS
# ------------------------------------------------------------

pois_bank <- feglm(
  breakthrough_count ~ ln_bank * ipr_c +
    ln_gdp_pc + ln_trade + ln_rd + ln_tertiary |
    iso3c + year,
  offset = log(patent_count),
  family = poisson(),
  data = df_est %>% filter(!is.na(ln_bank))
)
...
pois_vc <- feglm(
  breakthrough_count ~ ln_vc * ipr_c +
    ln_gdp_pc + ln_trade + ln_rd + ln_tertiary |
    iso3c + year,
  offset = log(patent_count),
  family = poisson(),
  data = df_est %>% filter(!is.na(ln_vc))
)

summary(pois_vc)

# ------------------------------------------------------------
# OVERDISPERSION CHECK (IMPORTANT)
# ------------------------------------------------------------

cat("\nOverdispersion diagnostic:\n")
cat("Mean breakthroughs:", mean(df_est$breakthrough_count), "\n")
cat("Variance breakthroughs:", var(df_est$breakthrough_count), "\n")

# If variance >> mean → move to Negative Binomial
```
