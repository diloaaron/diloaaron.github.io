...
```{r}
eq_hex <- ggplot() +
  # 1. Plot the hexbins first so they sit on the bottom layer
  geom_hex(data = master_earthquakes, aes(x = Longitude, y = Latitude), bins = 40) +
  
  # 2. Layer the county lines on top with a transparent fill
...
wq<- leaflet() %>%
  addProviderTiles(providers$CartoDB.DarkMatter) %>% # i like this background
  addPolygons(
    data = tx_counties,
